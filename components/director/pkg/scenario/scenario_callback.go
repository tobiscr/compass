...
import (
	...
	packageutil "github.com/kyma-incubator/compass/components/director/internal2/domain/package"
...
var (
	...
	basicAuthnDestination        = "BasicAuthentication"
...
func (c Callback) ScenarioCallback(ctx context.Context, _ interface{}, next graphql.Resolver) (res interface{}, err error) {
	...
	if err != nil {
		log.WithError(err).Errorf("An error occurred while opening the db transaction.")
		return nil, err
	}
	defer c.transact.RollbackUnlessCommitted(tx)
	...
	log.Infof("Fetching all applications for %q scenario BEFORE operation", c.scenarioName)
	appsBefore, err := c.fetchAppsInScenario(ctx, tenantID)
	if err != nil {
		return nil, err
	}
	log.Infof("Found %d applications for %q scenario BEFORE operation", len(appsBefore), c.scenarioName)

	log.Info("Proceeding with operation...")
	res, err = next(ctx)
	if err != nil {
		return res, err
	}
	...
	log.Infof("Fetching all applications for %q scenario AFTER operation", c.scenarioName)
	appsAfter, err := c.fetchAppsInScenario(ctx, tenantID)
	if err != nil {
		return nil, err
	}
	log.Infof("Found %d applications for %q scenario AFTER operation", len(appsAfter), c.scenarioName)
	...
	log.Infof("Found %d removed applications from scenario %q: %+v", len(removedApps), c.scenarioName, removedApps)
	...
	log.Infof("Found %d new applications from scenario %q: %+v", len(newApps), c.scenarioName, newApps)
	...
	for _, appName := range removedApps {
		...
		if err != nil {
			return nil, apperrors.NewInternalError(fmt.Sprintf("unable to construct deletion request %s:", err.Error()))
		}

		log.Infof("Proceeding with DELETE %s request...", url)
		resp, err := c.client.Do(req)
		if err != nil {
			return nil, apperrors.NewInternalError(fmt.Sprintf("unable to fetch destinations: %s", err.Error()))
		}

		if resp.StatusCode != http.StatusOK {
			log.Infof("Retrieved response status %d when trying to delete destination %q", resp.StatusCode, appName)
		} else {
			log.Infof("Successfully deleted destination %q", appName)
		}
	}
	...
	for _, app := range newApps {
		...
		log.Infof("Proceeding with POST %s request, body = %s", c.callbackURL, string(body))
		...
		if resp.StatusCode != http.StatusCreated {
			log.Infof("retrieved response status %d when trying to create destination %q", resp.StatusCode, app.Name)
		} else {
			log.Infof("Successfully created destination %q", app.Name)
		}
	}
...
func (c Callback) fetchAppsInScenario(ctx context.Context, tenantID string) (map[string]model.Application, error) {
	...
	if err != nil {
		return nil, apperrors.NewInternalError(fmt.Sprintf("unable to fetch runtimes for tenant with ID = %s", tenantID))
	}
	...
	if err != nil {
		return nil, apperrors.NewInternalError(fmt.Sprintf("unable to fetch applications for tenant with ID = %s", tenantID))
	}
