...
func TestUnmarshalBasicAuth(t *testing.T) {
	...
	basic, ok := a.Credential.(*externalschema.BasicCredentialData)
	require.True(t, ok)
...
func TestUnmarshalOAuth(t *testing.T) {
	...
	assert.Contains(t, scopes, "read")
	assert.Contains(t, scopes, "write")
	oauth, ok := a.Credential.(*externalschema.OAuthCredentialData)
	require.True(t, ok)
...
func TestUnmarshalCSRFBasicAuth(t *testing.T) {
	// GIVEN
	a := &externalschema.CSRFTokenCredentialRequestAuth{}
	// WHEN
	err := a.UnmarshalJSON([]byte(`{
		"credential": {
			"username": "aaa",
