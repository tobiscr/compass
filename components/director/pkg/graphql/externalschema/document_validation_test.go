...
func TestDocumentInput_Validate_Format(t *testing.T) {
	testCases := []struct {
		Name          string
		Value         externalschema.DocumentFormat
		ExpectedValid bool
	}{
		{
			Name:          "ExpectedValid",
...
func TestDocumentInput_Validate_FetchRequest(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			Name:          "Invalid object",
			Value:         &externalschema.FetchRequestInput{URL: "test-string"},
			ExpectedValid: false,
		},
	}
...
func fixValidDocument() externalschema.DocumentInput {
...
func fixCLOB(data string) *externalschema.CLOB {
	tmp := externalschema.CLOB(data)
	return &tmp
}
