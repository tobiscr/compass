...
// GetByExternalTenantName provides a mock function with given fields: ctx, externalTenantName
func (_m *TenantMappingRepository) GetByExternalTenantName(ctx context.Context, externalTenantName string) (*model.BusinessTenantMapping, error) {
	ret := _m.Called(ctx, externalTenantName)

	var r0 *model.BusinessTenantMapping
	if rf, ok := ret.Get(0).(func(context.Context, string) *model.BusinessTenantMapping); ok {
		r0 = rf(ctx, externalTenantName)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*model.BusinessTenantMapping)
		}
